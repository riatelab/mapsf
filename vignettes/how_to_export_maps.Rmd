...
mf_map(mtq, add = TRUE)
mf_title(txt = "PNG export: width=500px, height=615px (deduced)", cex = 1)
dev.off()
```
...
mf_map(mtq, add = TRUE)
mf_title(txt = "PNG export: width=500px, height=433px (deduced)")
dev.off()
mf_theme(NULL)
```
...
mf_export(
  x = target,
  filename = "fig/wo_export_fixed_height_centered.png",
  height = 600
)
mf_map(mtq, add = TRUE)
mf_shadow(target, add = TRUE)
mf_map(target, add = TRUE)
mf_title(txt = "PNG export: height=600px, width=425px (deduced)", cex = 1)
mf_scale(1, pos = "bottomleft")
dev.off()
```
![](fig/wo_export_fixed_height_centered.png)
...
mf_export(
  x = mtq,
  filename = "fig/wo_export_fixed_width.svg",
  width = 5,
  bg = "black"
)
mf_map(mtq, add = TRUE)
