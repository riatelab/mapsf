...
`mf_export()` is used to export maps in PNG or SVG formats. 
An exported map has the same width/height ratio of a spatial object, taking into account margins and title.   
...
In this export, the map is focused on a specific spatial object. `height` is fixed and `width` is deducted. 
