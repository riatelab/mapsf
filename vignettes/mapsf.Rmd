...
The `mf_init()` function can be used to export maps in raster (.png) or vector formats (.svg). The size of the exported map will fit the height/width ratio of a spatial object.
...
`mf_init()` is designed to initiate a map centered on a specific extent (`x` bbox). The `theme` argument allow to select a theme and the `shadow` argument add a shadow.
...
mtq <- mf_get_mtq()
# Initiate a base map
mf_init(x = mtq, shadow = TRUE, theme = "iceberg")
# plot municipalities 
...
mtq <- mf_get_mtq()
# set theme
mf_init(x = mtq, shadow = T, theme = "candy", expandBB = c(0,0,0,.15))
...
mtq <- mf_get_mtq()
# set theme
mf_init(x = mtq, shadow = T, theme = "ink", expandBB = c(0,0,0,.15))
# Plot the municipalities
mf_map(mtq, add = TRUE)
# Plot symbols with choropleth coloration
mf_map(
...
# set theme
mf_init(x = mtq, shadow = T, theme = "dark")
# Plot the municipalities
mf_map(mtq, add = TRUE)
