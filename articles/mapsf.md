...
- **Symbology**  
  [`mf_map()`](https://riatelab.github.io/mapsf/reference/mf_map.md) is
  the main function of the package, it displays map layers on a
  georeferenced plot. Many parameters are available to fine tune the
  cartographic representations. These parameters are the common ones
  found in GIS and automatic cartography tools (e.g. classification and
  color palettes used in choropleth maps, symbols sizes used in
...
[`mf_map()`](https://riatelab.github.io/mapsf/reference/mf_map.md) has
...
can be used to displays raster objects (single/multi band, continuous,
classes, intervals).
...
|  |  |
|----|----|
| ① [`mf_title()`](https://riatelab.github.io/mapsf/reference/mf_title.md) | Title of the map |
...
sets a map theme. A theme is a set of graphical parameters that are
applied to maps created with `mapsf`. These parameters are the figure
margins, background and foreground colors and
...
across maps produced by mapsf (e.g. they survive a
[`dev.off()`](https://rdrr.io/r/grDevices/dev.html) call).

🔗 See the online vignette: [How to use
themes](https://riatelab.github.io/mapsf/articles/how_to_use_themes)

### Export

[`mf_svg()`](https://riatelab.github.io/mapsf/reference/mf_svg.md) and
[`mf_png()`](https://riatelab.github.io/mapsf/reference/mf_png.md)
export maps in SVG and PNG formats, respectively. The exported map
...
maps](https://riatelab.github.io/mapsf/articles/how_to_export_maps)
...
is used to import a sample dataset as an `sf` object. Use
[`sf::st_read()`](https://r-spatial.github.io/sf/reference/st_read.html)
to import your own spatial datasets.

Map layout elements are set with
[`mf_title()`](https://riatelab.github.io/mapsf/reference/mf_title.md),
...
`expandBB` argument in
[`mf_map()`](https://riatelab.github.io/mapsf/reference/mf_map.md)
allows to expand the map space. Here, we increase the space available on
...
creates a link layer from an `sf` object and a link `data.frame` (long
format).
